...
import uk.gov.onelogin.criorchestrator.features.session.internal.usecases.GetJourneyTypeImpl
...
class GetJourneyTypeImplTest {
    @Test
    fun `when active session has redirect uri, it returns MAM journey`() = runTest {
    ...
    @Test
    fun `when active session does not have redirect uri, it returns DAD journey`() = runTest {
        val sessionStore =
            FakeSessionStore(
    ...
    @Test
    fun `when active session is null, expected crash occurs`() = runTest {
        val sessionStore =
            FakeSessionStore(
                session = null,
            )
