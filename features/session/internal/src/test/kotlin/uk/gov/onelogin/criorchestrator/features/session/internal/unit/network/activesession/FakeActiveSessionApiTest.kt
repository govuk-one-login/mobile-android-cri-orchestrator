package uk.gov.onelogin.criorchestrator.features.session.internal.unit.network.activesession
...
import uk.gov.onelogin.criorchestrator.features.session.internal.network.activesession.FakeActiveSessionApi
...
class FakeActiveSessionApiTest {
    ...
    @Test
    fun `given journey type should be MAM, it returns correct response`() = runTest {
        configStore.write(
            entry =
    ...
    @Test
    fun `given journey type should be DAD, it returns correct response`() = runTest {
        configStore.write(
            entry =
                Config.Entry<Config.Value.StringValue>(
                    key = SdkConfigKey.BypassJourneyType,
