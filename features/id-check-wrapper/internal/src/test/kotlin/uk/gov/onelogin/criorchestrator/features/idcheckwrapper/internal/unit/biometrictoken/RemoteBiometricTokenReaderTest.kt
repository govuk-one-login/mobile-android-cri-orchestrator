package uk.gov.onelogin.criorchestrator.features.idcheckwrapper.internal.unit.biometrictoken
...
import uk.gov.onelogin.criorchestrator.features.idcheckwrapper.internal.biometrictoken.RemoteBiometricTokenReader
...
class RemoteBiometricTokenReaderTest {
    ...
    @Test
    fun `emits offline when api response is a transport failure`() = runTest {
        whenever(
    ...
    @Test
    fun `emits error when api response is failure`() = runTest {
        whenever(
            biometricApi.getBiometricToken(SESSION_ID, documentType),
        ).thenReturn(
            ApiResponse.Failure(
    ...
    @Test
    fun `emits success when api response is success`() = runTest {
        val successData =
    ...
    @Test
    fun `emits error when api response is success but parsing fails`() = runTest {
        whenever(
            biometricApi.getBiometricToken(SESSION_ID, documentType),
