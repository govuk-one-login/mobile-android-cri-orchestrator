 ...
 */
abstract class ConfigKey<out T : Config.Value>(val name: String, val dependsOn: BooleanConfigKey? = null) {
...
abstract class BooleanConfigKey(name: String, dependsOn: BooleanConfigKey? = null) :
...
abstract class StringConfigKey(name: String, dependsOn: BooleanConfigKey? = null) :
    ConfigKey<Config.Value.StringValue>(name, dependsOn) {
    override fun requireValidValue(value: Config.Value) = require(value is Config.Value.StringValue) {
        "Value for $name must be a StringValue"
    }
}

abstract class OptionConfigKey(name: String, val options: ImmutableList<String>, dependsOn: BooleanConfigKey? = null) :
