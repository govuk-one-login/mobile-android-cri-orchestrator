package uk.gov.onelogin.criorchestrator.features.selectdoc.internal.component.drivinglicence.confirm
...
class ConfirmDrivingLicenceScreenAnalyticsTest {
    ...
    fun `when screen is started, it tracks analytics`() {
        val expectedEvent =
            ViewEvent
                .Screen(
                    id = SelectDocScreenId.ConfirmDrivingLicence.rawId,
                    name = context.getString(R.string.confirmdocument_drivinglicence_title),
                    params = SelectDocAnalytics.requiredParameters,
                ).asLegacyEvent()
        composeTestRule.setConfirmDrivingLicenceScreenContent()
    ...
    private fun ComposeContentTestRule.setConfirmDrivingLicenceScreenContent() {
        setContent {
